...
BOOL WindowImplBase::IsInStaticControl(CControlUI *pControl)
{
	...
	vctStaticName.push_back(_T("AnimationTabLayout"));
...
{// atler by yuyuming 2015-7-22
	CDuiString sCtrlName = msg.pSender->GetName();
	if( (sCtrlName == _T("closebtn")) && msg.sType == DUI_MSGTYPE_CLICK)
	{
		Close();
		return; 
	}
	else if( (sCtrlName == _T("minbtn")) && msg.sType == DUI_MSGTYPE_CLICK)
	{ 
		SendMessage(WM_SYSCOMMAND, SC_MINIMIZE, 0); 
		return; 
	}
	else if( (sCtrlName == _T("maxbtn")) && msg.sType == DUI_MSGTYPE_CLICK)
	{ 
		SendMessage(WM_SYSCOMMAND, SC_MAXIMIZE, 0); 
		return; 
	}
	else if( (sCtrlName == _T("restorebtn")) && msg.sType == DUI_MSGTYPE_CLICK)
	{ 
...
UINT WindowImplBase::DoModal()
{// add by yym 弹出模态对话框。ShowModal()接口存在漏洞。在此进行了修改。
// 模态对话框弹出后其他窗口的消息一律屏蔽掉，ShowModal()只屏蔽了同一父窗口的所有窗口消息。
	std::vector<HWND> vtEnableWindow;
	for (int i = 0; i < m_PaintManager.GetPreMessagesCount(); ++i)
	{
		HWND hWnd = m_PaintManager.GetPreMessagesAt(i)->GetPaintWindow();
			if (hWnd != NULL && hWnd != m_hWnd && IsWindowEnabled(hWnd))
			{
				vtEnableWindow.push_back(hWnd);
				EnableWindow(hWnd,FALSE);
			}
	}
	UINT nRet = ShowModal();
	for (size_t i = 0; i < vtEnableWindow.size(); ++i)
	{
		EnableWindow(vtEnableWindow[i],TRUE);
	}
	vtEnableWindow.clear();
	return nRet;
}
