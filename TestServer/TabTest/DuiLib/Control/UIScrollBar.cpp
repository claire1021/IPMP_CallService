...
namespace DuiLib
{
	CScrollBarUI::CScrollBarUI() : m_bHorizontal(false), m_nRange(100), m_nScrollPos(0), m_nLineSize(8), 
		...
		m_uButton2State(0), m_uThumbState(0), m_bShowButton1(true), m_bShowButton2(true),m_bHot1(false)
	{
	...
	LPCTSTR CScrollBarUI::GetButton1HotImage1()
	{
		return m_diButton1Hot1.sDrawString;
	}

	void CScrollBarUI::SetButton1HotImage1(LPCTSTR pStrImage)
	{
		if( m_diButton1Hot1.sDrawString == pStrImage && m_diButton1Hot1.pImageInfo != NULL ) return;
		m_diButton1Hot1.Clear();
		m_diButton1Hot1.sDrawString = pStrImage;
		Invalidate();
	}
	...
	LPCTSTR CScrollBarUI::GetButton2HotImage1()
	{
		return m_diButton2Hot1.sDrawString;
	}

	void CScrollBarUI::SetButton2HotImage1(LPCTSTR pStrImage)
	{
		if( m_diButton2Hot1.sDrawString == pStrImage && m_diButton2Hot1.pImageInfo != NULL ) return;
		m_diButton2Hot1.Clear();
		m_diButton2Hot1.sDrawString = pStrImage;
		Invalidate();
	}
	...
	LPCTSTR CScrollBarUI::GetThumbHotImage1()
	{
		return m_diThumbHot1.sDrawString;
	}

	void CScrollBarUI::SetThumbHotImage1(LPCTSTR pStrImage)
	{
		if( m_diThumbHot1.sDrawString == pStrImage && m_diThumbHot1.pImageInfo != NULL ) return;
		m_diThumbHot1.Clear();
		m_diThumbHot1.sDrawString = pStrImage;
		Invalidate();
	}
	...
	LPCTSTR CScrollBarUI::GetRailHotImage1()
	{
		return m_diRailHot1.sDrawString;
	}

	void CScrollBarUI::SetRailHotImage1(LPCTSTR pStrImage)
	{
		if( m_diRailHot1.sDrawString == pStrImage && m_diRailHot1.pImageInfo != NULL ) return;
		m_diRailHot1.Clear();
		m_diRailHot1.sDrawString = pStrImage;
		Invalidate();
	}
	...
	LPCTSTR CScrollBarUI::GetBkHotImage1()
	{
		return m_diBkHot1.sDrawString;
	}

	void CScrollBarUI::SetBkHotImage1(LPCTSTR pStrImage)
	{
		if( m_diBkHot1.sDrawString == pStrImage && m_diBkHot1.pImageInfo != NULL ) return;
		m_diBkHot1.Clear();
		m_diBkHot1.sDrawString = pStrImage;
		Invalidate();
	}
	...
	void CScrollBarUI::DoEvent(TEventUI& event)
	{
		...
		if( event.Type == UIEVENT_MOUSEMOVE )
		{
			if( (m_uThumbState & UISTATE_CAPTURED) != 0 ) {
				if( !m_bHorizontal ) {
					...
					//lxm,add，2015年8月3日16:17:22 取绝对值是因为出现负值时，导致鼠标拖动时，滚动条运动方向不正确
					vRange = abs(vRange);
		...
		if( event.Type == UIEVENT_TIMER )
		{
			...
			else if(event.wParam == DEFAULT_TIMERID_UNHOT)
			{
				m_pManager->KillTimer(this, DEFAULT_TIMERID_UNHOT);
				m_bHot1 = false;
				Invalidate();
				return;
			}
		...
		if( event.Type == UIEVENT_MOUSELEAVE )
		{
			if( IsEnabled() ) {
				...
				if((m_uButton1State & UISTATE_PUSHED) == 0 &&
					(m_uButton2State & UISTATE_PUSHED) == 0 &&
	...
	void CScrollBarUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		...
		else if( _tcscmp(pstrName, _T("button1hotimage1")) == 0 ) SetButton1HotImage1(pstrValue); 
		...
		else if( _tcscmp(pstrName, _T("button2hotimage1")) == 0 ) SetButton2HotImage1(pstrValue);
		...
		else if( _tcscmp(pstrName, _T("thumbhotimage1")) == 0 ) SetThumbHotImage1(pstrValue);
		...
		else if( _tcscmp(pstrName, _T("railhotimage1")) == 0 ) SetRailHotImage1(pstrValue);
		...
		else if( _tcscmp(pstrName, _T("bkhotimage1")) == 0 ) SetBkHotImage1(pstrValue);
	...
	void CScrollBarUI::PaintBk(HDC hDC)
	{
		...
		if( m_bHot1){
			if(DrawImage(hDC, m_diBkHot1)) return;
		} 
	...
	void CScrollBarUI::PaintButton1(HDC hDC)
	{
		...
		if(m_bHot1)
		{
			m_diButton1Hot1.rcDestOffset = rc;
			if( DrawImage(hDC, m_diButton1Hot1) ) return;
		}

		m_diButton1Normal.rcDestOffset = rc;
		if( DrawImage(hDC, m_diButton1Normal) ) return;

		//lxm,delete,2015年7月10日13:38:12
		//原为当没有设置图片时绘制一个矩形把该功能去掉
...
// 		CRenderEngine::DrawRect(hDC, m_rcButton1, nBorderSize, dwBorderColor);
	...
	void CScrollBarUI::PaintButton2(HDC hDC)
	{
		...
		if(m_bHot1)
		{
			m_diButton2Hot1.rcDestOffset = rc;
			if( DrawImage(hDC, m_diButton2Hot1) ) return; 
		}
...
// 		CRenderEngine::DrawRect(hDC, m_rcButton2, nBorderSize, dwBorderColor);
	...
	void CScrollBarUI::PaintThumb(HDC hDC)
	{
		...
		if(m_bHot1)
		{
			m_diThumbHot1.rcDestOffset = rc;
			if( DrawImage(hDC, m_diThumbHot1) ) return; 
		}
...
// 		CRenderEngine::DrawRect(hDC, m_rcThumb, nBorderSize, dwBorderColor);
	...
	void CScrollBarUI::PaintRail(HDC hDC)
	{
		...
		if(m_bHot1)
		{
			m_diRailHot1.rcDestOffset = rc;
			if( DrawImage(hDC, m_diRailHot1) ) return; 
		}
		m_diRailNormal.rcDestOffset = rc;
		if( DrawImage(hDC, m_diRailNormal) ) return;
	}

	//设置为hot状态，5秒后自动恢复normal状态
	void CScrollBarUI::SetHot()
	{
		m_bHot1 = true;
		Invalidate();
		m_pManager->SetTimer(this, DEFAULT_TIMERID_UNHOT, 5000U);
	}

	bool CScrollBarUI::GetHotState()
	{
		return m_bHot1;
	}
