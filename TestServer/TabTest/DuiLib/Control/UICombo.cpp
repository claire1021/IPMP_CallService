...
LRESULT CComboWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    if( uMsg == WM_CREATE ) {
        ...
        LPCTSTR pDefaultAttributes = m_pOwner->GetManager()->GetDefaultAttributeList(_T("VerticalLayout"),true);
        if( pDefaultAttributes ) {
            m_pLayout->ApplyAttributeList(pDefaultAttributes);
        }
		//lxm,added,2015年12月9日，使用窗口滚动条属性
		pDefaultAttributes = m_pOwner->GetManager()->GetDefaultAttributeList(_T("VScrollBar"),true);
		if(pDefaultAttributes)
			m_pm.AddDefaultAttributeList(_T("VScrollBar"),pDefaultAttributes);
		pDefaultAttributes = m_pOwner->GetManager()->GetDefaultAttributeList(_T("HScrollBar"),true);
		if(pDefaultAttributes)
			m_pm.AddDefaultAttributeList(_T("HScrollBar"),pDefaultAttributes);
		//lxm,added,2015年12月9日，end
        ...
        return 0;
		 ...
		 //lxm,delete,这一段在窗口关闭时修改了窗口位置，导致combox在选择一次后位置发生变化
// 		CControlUI* pParent = m_pOwner->GetParent();
// 		if(pParent != NULL)
// 		{
// 			RECT rcParentPos = pParent->GetPos();
// 			RECT rc = m_pOwner->GetPos();
// 			RECT rcCtrl = {rc.left - rcParentPos.left, rc.top - rcParentPos.top, rc.right - rcParentPos.left, rc.bottom - rcParentPos.top};
// 			m_pOwner->SetPos(rcCtrl, false);
// 		}
		...
		if( m_pScroll)
		{ 
			//lxm,added, 如果点击滚动条，记录当前滚动条位置 
			RECT rcItem = m_pLayout->GetItemAt(0)->GetPos();
			RECT rcList = m_pLayout->GetPos();
			int nCount = m_pOwner->GetCount() - (rcList.bottom - rcList.top)/(rcItem.bottom - rcItem.top);
			m_nCurTop = m_pScroll->GetScrollPos()/(m_pScroll->GetScrollRange()/nCount); 
		}
		else
		{
	...
	else if (uMsg == WM_LBUTTONDOWN)
	{
		...
		{
			m_pScroll = (CScrollBarUI * )pControl;
		}
		else
		{
			m_pScroll = NULL;
		...
		//lxm,chenged,直接滚动窗口 不选中
        int zDelta = (int) (short) HIWORD(wParam); 
		if(zDelta > 0 )
			EnsureVisible(m_nCurTop - 1);
		else if(zDelta < 0 )
			EnsureVisible(m_nCurTop + 1);
...
void CComboWnd::EnsureVisible(int iIndex)
{
	...
	if(iIndex < 0 || iIndex > m_pOwner->GetCount() - 1) 
		return;
    m_pLayout->FindSelectable(m_pOwner->GetCurSel(), false);
    RECT rcItem = m_pLayout->GetItemAt(iIndex)->GetPos();
    RECT rcList = m_pLayout->GetPos();
	if(iIndex > m_pOwner->GetCount() - (rcList.bottom - rcList.top)/(rcItem.bottom - rcItem.top))
		return;
...
void CComboUI::DoEvent(TEventUI& event)
{
	...
	{     if( IsEnabled() ) { 
		//lxm,chenged,2015年9月20日，改为鼠标弹起时弹出列表，
		//manager中为了点击按钮时，避免某些控件失焦点时软键盘关闭，窗口移动位置，导致按钮点击不中
		//将焦点的切换从鼠标按下改为鼠标弹起，这样鼠标弹起时弹出就会因为失焦点关闭，所以把弹出窗口从鼠标按下改为
		//鼠标弹起中
			Activate();
...
void CComboUI::SetPos(RECT rc, bool bNeedInvalidate)
{
	...
	{ 
		...
		if( pParent != NULL ) {
			RECT rcParentPos = pParent->GetPos();
			if(rc.left < rcParentPos.left || rc.top < rcParentPos.top)
			{
				CControlUI::SetPos(rc, bNeedInvalidate);
			}
			else
			{
				RECT rcCtrl = {rc.left - rcParentPos.left ,   rc.top - rcParentPos.top, 
					rc.right - rcParentPos.left,  rc.bottom -  rcParentPos.top };

				CControlUI::SetPos(rcCtrl, bNeedInvalidate);
			}
		}
	} 
	else
