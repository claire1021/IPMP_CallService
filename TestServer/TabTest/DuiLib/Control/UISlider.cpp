...
namespace DuiLib
{
	CSliderUI::CSliderUI() : m_uButtonState(0), m_nStep(1), m_bSpecialThumb(false)
	{
	...
	void CSliderUI::DoEvent(TEventUI& event)
	{
		...
		if( event.Type == UIEVENT_BUTTONUP )
		{
			...
			if( m_bHorizontal ) {
				...
				//lxm,chenged,2015年11月26日，提高滑块跟随鼠标的精准度
				...
				else  m_nValue = m_nMin + (m_nMax - m_nMin) * (event.ptMouse.x - m_rcItem.left - m_szThumb.cx / 2 ) / (m_rcItem.right - m_rcItem.left - m_szThumb.cx) + ((m_nMax - m_nMin) *(event.ptMouse.x - m_rcItem.left - m_szThumb.cx / 2 ) % (m_rcItem.right - m_rcItem.left - m_szThumb.cx) >=(m_rcItem.right - m_rcItem.left - m_szThumb.cx)/2?1:0);
				//lxm,end 
			}
			else {
				...
				else m_nValue = m_nMin + (m_nMax - m_nMin) * (m_rcItem.bottom - event.ptMouse.y - m_szThumb.cy / 2 ) / (m_rcItem.bottom - m_rcItem.top - m_szThumb.cy) +( (m_nMax - m_nMin) * (m_rcItem.bottom - event.ptMouse.y - m_szThumb.cy / 2 ) %(m_rcItem.bottom - m_rcItem.top - m_szThumb.cy) >= (m_rcItem.bottom - m_rcItem.top - m_szThumb.cy)/2?1:0);
				//lxm,end 
			}
			
			//del lht
			//m_pManager->SendNotify(this, DUI_MSGTYPE_VALUECHANGED);
			m_pManager->SendNotify(this, DUI_MSGTYPE_SLIDERUP);
			//end del
	...
	void CSliderUI::SetSpecialSlider( bool bSpecial )
	{
		m_bSpecialThumb = bSpecial;
	}
