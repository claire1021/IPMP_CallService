...
// 				if( pHorizontalScrollBar && pHorizontalScrollBar->IsVisible() ) rc.bottom -= pHorizontalScrollBar->GetFixedHeight();
	...
	bool CTreeNodeUI::AddChildNode( CTreeNodeUI* _pTreeNodeUI )
	{
		...
		//lxm,chenged,2015年8月24日
		return mTreeNodes.Add(_pTreeNodeUI);
//		bool nRet = true;
...
//		return nRet;
	...
	bool CTreeViewUI::Add( CTreeNodeUI* pControl )
	{
		...
		//lxm,added, 根据树的属性，修改节点属性
		pControl->SetItemTextColor(m_dwItemTextColor);
		pControl->SetItemHotTextColor(m_dwItemHotTextColor);
		pControl->SetSelItemTextColor(m_dwSelItemTextColor);
		pControl->SetSelItemHotTextColor(m_dwSelItemHotTextColor);
		//end
	...
	bool CTreeViewUI::AddAt( CTreeNodeUI* pControl,CTreeNodeUI* _IndexNode )
	{
		...
		//lxm,added, 根据树的属性，修改节点属性
		_IndexNode->SetItemTextColor(m_dwItemTextColor);
		_IndexNode->SetItemHotTextColor(m_dwItemHotTextColor);
		_IndexNode->SetSelItemTextColor(m_dwSelItemTextColor);
		_IndexNode->SetSelItemHotTextColor(m_dwSelItemHotTextColor);
		//end
	...
	void CTreeViewUI::SetAttribute( LPCTSTR pstrName, LPCTSTR pstrValue )
	{
		if(_tcscmp(pstrName,_T("visiblefolderbtn")) == 0)
			...
			if( *pstrValue == _T('#')) pstrValue = ::CharNext(pstrValue);
			LPTSTR pstr = NULL;
			m_dwItemTextColor = _tcstoul(pstrValue, &pstr, 16);
			SetItemTextColor(m_dwItemTextColor);
		}
		else if(_tcscmp(pstrName, _T("itemhottextcolor")) == 0 ){
			if( *pstrValue == _T('#')) pstrValue = ::CharNext(pstrValue);
			LPTSTR pstr = NULL;
			m_dwItemHotTextColor = _tcstoul(pstrValue, &pstr, 16);
			SetItemHotTextColor(m_dwItemHotTextColor);
		}
		else if(_tcscmp(pstrName, _T("selitemtextcolor")) == 0 ){
			if( *pstrValue == _T('#')) pstrValue = ::CharNext(pstrValue);
			LPTSTR pstr = NULL;
			m_dwSelItemTextColor = _tcstoul(pstrValue, &pstr, 16);
			SetSelItemTextColor(m_dwSelItemTextColor);
		}
		else if(_tcscmp(pstrName, _T("selitemhottextcolor")) == 0 ){
			if( *pstrValue == _T('#')) pstrValue = ::CharNext(pstrValue);
			LPTSTR pstr = NULL;
			m_dwSelItemHotTextColor = _tcstoul(pstrValue, &pstr, 16);
			SetSelItemHotTextColor(m_dwSelItemHotTextColor);
		}
