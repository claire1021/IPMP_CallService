...
namespace DuiLib
{
	...
	void CButtonUI::DoEvent(TEventUI& event)
	{
		...
		if( event.Type == UIEVENT_TOUCHDOWN )
		{
			if( ::PtInRect(&m_rcItem, event.ptMouse) && IsEnabled() ) {
				m_uButtonState |= UISTATE_PUSHED | UISTATE_CAPTURED;
				m_pManager->SendNotify(this, DUI_MSGTYPE_TOUCHDOWN, event.wParam, event.lParam);
				Invalidate();
			}
			//return;
		}
		if( event.Type == UIEVENT_TOUCHUP )
		{
			if( (m_uButtonState & UISTATE_CAPTURED) != 0 ) {
				//因为会执行鼠标消息，为了不重复执行，不让程序执行touch消息，响应touch只是为了显示状态改变
				//if( ::PtInRect(&m_rcItem, event.ptMouse) )  	Activate(); 
				m_uButtonState &= ~(UISTATE_PUSHED | UISTATE_CAPTURED);
				m_pManager->SendNotify(this, DUI_MSGTYPE_TOUCHUP, event.wParam, event.lParam);
				Invalidate();
			}
			//return;
		}
	...
	void CButtonUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		...
		else if(_tcscmp(pstrName, _T("verticaltext")) == 0)
		{
			if(_tcscmp(pstrValue, "true") == 0)
	...
	void CButtonUI::PaintText(HDC hDC)
	{
		...
		else
		{
			...
			nGraphics.TranslateTransform((float)((rc.left + rc.top + rc.right + rc.bottom)/2),
				(float)((rc.top + rc.bottom - rc.left - rc.right)/2) );
			nGraphics.RotateTransform(90);
			clrColor |= 0xFF000000;
			SolidBrush brush(clrColor);  
			RectF nRc((float)(rc.left - (rc.bottom + rc.left - rc.top - rc.right)/2),
				(float)(rc.top + (rc.bottom + rc.left - rc.top - rc.right)/2),
				(float)rc.bottom - rc.top,(float)rc.right-rc.left);
#ifdef _UNICODE
			nGraphics.DrawString(m_sText, -1/*lstrlen(m_strText)*/, &font, _rcText, &stringformat, &brush); 
#else
			USES_CONVERSION; 
			SolidBrush b(Color::Red); 
			wstring mText = A2W(m_sText.GetData());
			nGraphics.DrawString(mText.c_str(), mText.length(), &nFont, nRc, &format, &brush); 
#endif  
		}
	}
