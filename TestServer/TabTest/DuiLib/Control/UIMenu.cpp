...
// 返回值:   	LRESULT		
//*******************************************************************
LRESULT CMenuWnd::OnButtonUp( UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled )
{
	...
	if( pControl != NULL &&pControl->GetManager()->GetPaintWindow() == m_hWnd) 
	{
		TEventUI event = { 0 };
...
LRESULT CMenuWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_RBUTTONUP:
	case WM_LBUTTONUP:
		lRes = OnButtonUp(uMsg, wParam, lParam, bHandled);
		return 0;
		break;	
	default:
		bHandled = FALSE;
