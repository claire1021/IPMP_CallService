...
HRESULT CTxtWinHost::TxNotify(DWORD iNotify, void *pv)
{
	...
	{ 
		if( m_re == NULL ) return 0;

		CDuiString str = m_re->GetText();//static_cast<LPTSTR>(_alloca(cchLen * sizeof(TCHAR)));
		if( str == _T("") ) return 0;  

		int nMutLen = str.GetLength();
		if((nMutLen) > (int) m_re->GetLimitText())
		{  
			//获取光标位置 
			long start, end;
			m_re->GetSel(start,end);
			int nPos = start;
			if (0 == strlen(m_strlast.c_str()))
			{  
				int count = 0;
				int i = 0;
				for (i = 0; i<m_re->GetLimitText(); i++)
				{
					if ((str.GetAt(i)&0x80) == 0x80)
						count++;
				}
				if (0 == count%2)  
					str.SetAt(m_re->GetLimitText(),'\0');  
				else 
					str.SetAt(m_re->GetLimitText() -1, '\0'); 
				m_strlast = str;   
			}
			m_re->SetText(m_strlast.c_str()); 
			m_re->SetSel(nPos -1, nPos -1);
		} 
		else
		{ 
			m_strlast = "";
			if(nMutLen == m_re->GetLimitText())
			{
				m_strlast = str;   
			} 
		}  
...
CRichEditUI::CRichEditUI() : m_pTwh(NULL), m_bVScrollBarFixing(false), m_bWantTab(true), m_bWantReturn(true), 
    ...
    m_iLimitText(cInitTextMax), m_lTwhStyle(ES_MULTILINE), m_bInited(false),m_bShowKeyboard(true),m_strSetFocusControlName (_T(""))
	,m_strKillFocusControlName  (_T(""))
{
}
...
void CRichEditUI::OnTxNotify(DWORD iNotify, void *pv)
{
	...
	switch(iNotify)
	{ 
		//wjh 2016012 add begin
	case EN_SETFOCUS:
		m_strKillFocusControlName =_T("");
		m_strSetFocusControlName = this->GetName();
		break;
	case EN_KILLFOCUS:
		m_strKillFocusControlName = this->GetName();
		break;
...
// 			POINT pt = { rc.left +  rc.right/2, rc.right + rc.left/2  };
// 			::ScreenToClient(GetManager()->GetPaintWindow(), &pt);
// 			CControlUI* pHover = GetManager()->FindControl(pt);
// 			if(pHover == this)
			if((m_strSetFocusControlName == this->GetName()) && (m_strKillFocusControlName != this->GetName()))
...
void CRichEditUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
	...
	//lxm,added,2015年9月25日，添加rich最大字符限制
...
// 			CControlUI* pHover = GetManager()->FindControl(pt);
// 			  if(pHover == this)
// 				GetManager()->SendNotify(pHover, DUI_MSGTYPE_TEXTCHANGED,wParam);
// 		}
...
void CRichEditUI::ShowKeyBoard( bool bShow )
{
