...
void CListUI::DoEvent(TEventUI& event)
{
    ...
    switch( event.Type ) {
 	case UIEVENT_BUTTONUP:
 		m_pList->ButtonUp(event.ptMouse);
 		break;
 	case UIEVENT_MOUSEMOVE: 
 		m_pList->MouseMove(event.ptMouse);
 		break;
	case UIEVENT_BUTTONDOWN:
		m_pList->ButtonDown(event.ptMouse);
	case UIEVENT_DBLCLICK:
		m_pList->SetScrollBarHot();
		break;
...
void CListUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
	...
	else if (_tcscmp(pstrName, _T("itemselected")) == 0 )
	{
		if( _tcscmp(pstrValue, _T("true")) == 0 )
			m_bSupportSelected = true;
		else
			m_bSupportSelected = false;
	}
...
void CListBodyUI::SetScrollPos(SIZE szPos)
{
    int cx = 0;
    int cy = 0;
	//lxm,deleted,2015年8月22日,这里判断目的点为0则不滚动是错误的
	...
	if(m_pManager != NULL)
	{
		m_pManager->SendNotify(m_pOwner, DUI_MSGTYPE_SCROLL,cx,cy);
	}
}
...
//     if( m_pHorizontalScrollBar && m_pHorizontalScrollBar->IsVisible() ) rc.bottom -= m_pHorizontalScrollBar->GetFixedHeight();
	...
	if(m_pHorizontalScrollBar != NULL)
		cyNeeded += m_pHorizontalScrollBar->GetFixedHeight();
...
void CListBodyUI::ButtonDown( CDuiPoint pt )
{
		m_nLastX = pt.x;
		m_nLastY = pt.y;
		m_bLButtonDown = true;
...
void CListBodyUI::ButtonUp( CDuiPoint pt )
{
	m_bLButtonDown = false;
}
...
//lxm,added,添加随鼠标滚动界面
void CListBodyUI::MouseMove( CDuiPoint pt )
{
	if(m_bLButtonDown)
	{ 
		if(m_pVerticalScrollBar != NULL && m_pVerticalScrollBar->IsVisible() && m_pVerticalScrollBar->IsEnabled()&& pt.y >= 0)
		{
		...
		if( m_pHorizontalScrollBar != NULL && m_pHorizontalScrollBar->IsVisible() && m_pHorizontalScrollBar->IsEnabled()&& pt.x >= 0 )
		{
			SIZE sz = GetScrollPos();
			sz.cx -= (pt.x - m_nLastX);
			SetScrollPos(sz);
...
void CListElementUI::Invalidate()
{
    ...
    if( GetParent() ) {
        ...
        if( pParentContainer ) {
			...
			//lxm,delete.2015年7月31日12:27:08，修改list鼠标滑动时滚动条部分背景不更新的问题，当使用透明滚动条时会用到
...
void CListLabelElementUI::DrawItemText(HDC hDC, const RECT& rcItem)
{
    ...
    if( pInfo->bShowHtml )
	{//wjh修改，增加DrawHtmlText的重载函数
		if(m_bCenter)
			CRenderEngine::DrawHtmlText(hDC, m_pManager, rcText, m_sText, iTextColor, \
			NULL, NULL, nLinks, DT_SINGLELINE | pInfo->uTextStyle,m_iHeight);
		else
			CRenderEngine::DrawHtmlText(hDC, m_pManager, rcText, m_sText, iTextColor, \
...
//wjh修改，设置居中和高度
void CListLabelElementUI::SetCenterAndHeight(bool bCenter,int iHeight)
{
	m_iHeight = iHeight;
	m_bCenter = bCenter;
}
