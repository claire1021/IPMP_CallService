...
protected:
	...
	//add lht
	//如果字段为true，则list为非选中list
	bool	m_bSupportSelected;
};
...
public:
    ...
    void DoEvent(TEventUI& event);
    BOOL SortItems(PULVCompareFunc pfnCompare, UINT_PTR dwData);
	void ButtonDown(CDuiPoint pt);
	void ButtonUp(CDuiPoint pt); 
	void MouseMove(CDuiPoint pt);
protected:
...
public:
	...
	//wjh2015/07/16 增加成员函数，处理两变量
	void SetCenterAndHeight(bool bCenter,int iHeight);
private:

	//wjh2015/07/16 ,增加两个成员变量
	bool m_bCenter;
	int m_iHeight; 
};
