...
{
	if (!IsEnabled() && event.Type == UIEVENT_DBLCLICK)
	{
		Invalidate();
		return;
	}
...
// 		ASSERT(rcPos.bottom - rcPos.top >= 0);
	...
	//lxm,added,2015年7月9日16:24:28
	//添加colorvertical属性，true为竖直，false为水平
	else if(_tcscmp(pstrName, _T("colorvertical")) == 0)
	{
		if(_tcscmp(pstrValue, _T("true")) == 0 || _tcscmp(pstrValue, _T("TRUE")) == 0)
		{
			SetBkVertical(true);
		}
		else
    ...
    if( m_dwBackColor != 0 ) {
        if( m_dwBackColor2 != 0 ) {
            if( m_dwBackColor3 != 0 ) {
				...
				//lxm,chenged,2015年7月9日16:20:03
				//CRenderEngine::DrawGradient(hDC, rc, GetAdjustColor(m_dwBackColor), GetAdjustColor(m_dwBackColor2), true, 8); 
                CRenderEngine::DrawGradient(hDC, rc, GetAdjustColor(m_dwBackColor), GetAdjustColor(m_dwBackColor2), m_bBkVertical, 8);
                rc.top = rc.bottom;
                rc.bottom = m_rcItem.bottom; 
				//lxm,chenged,2015年7月9日16:20:03
				//CRenderEngine::DrawGradient(hDC, rc, GetAdjustColor(m_dwBackColor2), GetAdjustColor(m_dwBackColor3), true, 8);
                CRenderEngine::DrawGradient(hDC, rc, GetAdjustColor(m_dwBackColor2), GetAdjustColor(m_dwBackColor3), m_bBkVertical, 8);
            }
			else 
				//lxm,chenged,2015年7月9日16:20:03
				// CRenderEngine::DrawGradient(hDC, m_rcItem, GetAdjustColor(m_dwBackColor), GetAdjustColor(m_dwBackColor2), true, 16);
                CRenderEngine::DrawGradient(hDC, m_rcItem, GetAdjustColor(m_dwBackColor), GetAdjustColor(m_dwBackColor2), m_bBkVertical, 16);
        }
...
void CControlUI::SetBkVertical( bool bVertical )
{
	m_bBkVertical = bVertical;
} 
} // namespace DuiLib
