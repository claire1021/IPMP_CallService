...
//
//
	const int KEYBOARD_TIMER_ID = 888;
	const int KEYBOARD_CLOSE_DELAY = 200;
...
/*BOOL CPaintManagerUI::m_bLButtonDown = FALSE;*/

HWND CPaintManagerUI::m_hKeyBoard = NULL;
...
#ifdef USE_KEYBOARD 
	//lxm,打开软键盘
	if(m_aPreMessages.GetSize() == 0)
	{
		//判断是否为触屏
		BYTE digitizerStatus = (BYTE)GetSystemMetrics(SM_DIGITIZER);
		if((digitizerStatus & (0x80 + 0x40)) != 0)
		{ 
			m_hKeyBoard = ::FindWindow(NULL, KEYBOARD_NAME);
			if(m_hKeyBoard == NULL)
			{
				//启动软键盘
				CDuiString strExeFolderPath; 
				TCHAR s[1024];
				GetModuleFileName(NULL,s,1024);  
				strExeFolderPath = s;
				int nPos;  
				nPos=strExeFolderPath.ReverseFind('\\');    
				strExeFolderPath=strExeFolderPath.Left(nPos);
				strExeFolderPath += KEYBOARD_APP; 
				ShellExecute(m_hWndPaint, _T("open"),strExeFolderPath,NULL, NULL,SW_SHOWNORMAL  ); 
				for(int i = 0; i < 100,m_hKeyBoard == NULL; i++)
				{ 
					m_hKeyBoard = ::FindWindow(NULL, KEYBOARD_NAME);
					Sleep(50);
				}  
...
CPaintManagerUI::~CPaintManagerUI()
{
	...
	UnregisterTouchWindow(m_hWndPaint);
	...
	{ 
		...
		MoveWindow(m_hWndPaint,rcWnd.left, m_rcWidnow.top, rcWnd.GetWidth(), m_rcWidnow.GetHeight(), false);
		COPYDATASTRUCT cpd; // 给COPYDATASTRUCT结构赋值

		cpd.dwData = HIDE_KEYBORAD;

		cpd.cbData = 0;

		cpd.lpData = NULL;
		BOOL Result = ::SendMessage(m_hKeyBoard, WM_COPYDATA, (WPARAM)m_hWndPaint,(LPARAM)&cpd) ; 
	}
	//lxm,added,关闭软键盘
	if(m_hKeyBoard != NULL && m_aPreMessages.GetSize() == 0)
	{
		COPYDATASTRUCT cpd; // 给COPYDATASTRUCT结构赋值

		cpd.dwData = CLOSE_KEYBORAD;

		cpd.cbData = 0;

		cpd.lpData = NULL;
		BOOL Result = ::SendMessage(m_hKeyBoard, WM_COPYDATA, NULL,(LPARAM)&cpd) ;
		m_hKeyBoard = NULL;
	}
...
void CPaintManagerUI::Init(HWND hWnd)
{
	...
	//注册触屏消息
	BOOL b = RegisterTouchWindow(m_hWndPaint,TWF_FINETOUCH);
	int a = GetLastError();
	int c = 0;
}
...
//#endif
					...
					//修改刷新时,Pos出现问题
                     else 
					 {
						 ...
						 {							  
 							...
 							/// if( _tcsstr(pControl->GetClass(), _T("ComboUI")) != NULL )
 							///	 return true;
    ...
    case WM_LBUTTONUP:
        {
			...
			//lxm,added,
			//避免某些控件失焦点时软键盘关闭，窗口移动位置，导致按钮点击不中
				//将焦点的切换从鼠标按下改为鼠标弹起，
			m_pEventClick->SetFocus();
			//end
            ReleaseCapture();
            TEventUI event = { 0 };
            event.Type = UIEVENT_BUTTONUP;
	...
	case WM_TOUCH:
		{
		//	::SendMessage(m_hWndPaint, WM_TOUCH, wParam, lParam);
...
LPCTSTR CPaintManagerUI::GetDefaultAttributeList(LPCTSTR pStrControlName,bool bShare) const
{
	//lxm,chenged,2016年4月19日，子控件获取父窗口属性时首先选择具有share属性的value如果没有在选择私有的
...
// 	return NULL;
	...
	if(bShare)
	{
		...
		if( !pDefaultAttr ) pDefaultAttr = static_cast<CDuiString*>(m_ResInfo.m_AttrHash.Find(pStrControlName));
	}
	else
	{
...
int CPaintManagerUI::GetPreMessagesCount()
{
	return m_aPreMessages.GetSize();
}

CPaintManagerUI * CPaintManagerUI::GetPreMessagesAt( UINT nIndex )
{
	if (nIndex >= (UINT)m_aPreMessages.GetSize())
	{
		return NULL;
	}
	return static_cast<CPaintManagerUI *>(m_aPreMessages[nIndex]);
}

void CPaintManagerUI::ShowKeyBoard( bool bShow, CControlUI * pControl/* = NULL*/ )
{
	//判断是否为触屏
// 	BYTE digitizerStatus = (BYTE)GetSystemMetrics(SM_DIGITIZER);
// 	if((digitizerStatus & (0x80 + 0x40)) == 0) {
...
// 	}
	...
	{  
		...
		{  
			COPYDATASTRUCT cpd; // 给COPYDATASTRUCT结构赋值 
			cpd.dwData = SHOW_KEYBORAD; 
			cpd.cbData = 0; 
			cpd.lpData = NULL;
			BOOL Result = ::SendMessage(m_hKeyBoard, WM_COPYDATA, (WPARAM)GetPaintWindow(),(LPARAM)&cpd) ; 
			Sleep(100);
			CDuiRect rcWnd;
			GetWindowRect(GetPaintWindow(), &rcWnd);
			//记录初始位置
			if(m_rcWidnow.top == m_rcWidnow.bottom)
				m_rcWidnow = rcWnd; 
			//获取屏幕信息
			MONITORINFO oMonitor = {};
			oMonitor.cbSize = sizeof(oMonitor);
			::GetMonitorInfo(::MonitorFromWindow(GetPaintWindow(), MONITOR_DEFAULTTONEAREST), &oMonitor); 
			if(rcWnd.bottom > (oMonitor.rcMonitor.bottom - oMonitor.rcMonitor.top)*3/5)
			{
				if(m_bMoveWithKeyBoard )//自动挪动，并且没有挪动过
				{   
					//窗口上移
					MoveWindow(GetPaintWindow(),rcWnd.left, (oMonitor.rcMonitor.bottom - oMonitor.rcMonitor.top)*3/5 - rcWnd.GetHeight(),rcWnd.GetWidth(), rcWnd.GetHeight(), false);
				} 
				else if(pControl != NULL)
				{
					//获取当前控件位置，如果被屏幕挡住，即使窗体没有自动移动属性，也需要挪动把编辑框显示出来
					//获取控件位置
					CDuiRect rcControl = pControl->GetPos();
					//计算控件相对于屏幕的底部位置
					int nBottom = rcControl.bottom + rcWnd.top; 
					if(nBottom  > (oMonitor.rcMonitor.bottom - oMonitor.rcMonitor.top)*3/5)
						MoveWindow(GetPaintWindow(), rcWnd.left, (oMonitor.rcMonitor.bottom - oMonitor.rcMonitor.top)*3/5 - rcControl.bottom, rcWnd.GetWidth(), rcWnd.GetHeight(), false);
				}
			}
			OutputDebugString("imms_key_open"); 
		}
	}
	else
	{ 
		if(m_hKeyBoard != NULL)
		{
			SetTimer(NULL, KEYBOARD_TIMER_ID, KEYBOARD_CLOSE_DELAY);//延时关闭
			OutputDebugString("imms_key_close"); 
		}
	}
#endif
}

void CPaintManagerUI::SetAutoMove( bool bAuto )
{
	m_bMoveWithKeyBoard = bAuto;
}

void CPaintManagerUI::OnTimer(UINT uWinTimer )
{
	for( int i = 0; i < m_aTimers.GetSize(); i++ ) 
	{
		const TIMERINFO* pTimer = static_cast<TIMERINFO*>(m_aTimers[i]);
		if(pTimer->pSender == NULL)
		{ 
			if(pTimer->nLocalID  == KEYBOARD_TIMER_ID && !pTimer->bKilled)
			{
				KillTimer(NULL, KEYBOARD_TIMER_ID); 
#ifdef USE_KEYBOARD 
					if(!m_bShowKeyboard)
				{ 
					CDuiRect rcWnd;
					GetWindowRect(GetPaintWindow(), &rcWnd);
					MoveWindow(GetPaintWindow(),rcWnd.left, m_rcWidnow.top, rcWnd.GetWidth(), m_rcWidnow.GetHeight(), false);
					if(m_hKeyBoard!= NULL)
					{ 
						COPYDATASTRUCT cpd; // 给COPYDATASTRUCT结构赋值
		...
		else if( pTimer->hWnd == m_hWndPaint && pTimer->uWinTimer == LOWORD(uWinTimer) && pTimer->bKilled == false) 
		{
			TEventUI event = { 0 };
...
// 
// BOOL CPaintManagerUI::IsLButtonDown()
// {
