        ...
        if( _tcsicmp(pstrClass, _T("Window")) == 0 ) {
            if( pManager->GetPaintWindow() ) {
                ...
                for( int i = 0; i < nAttributes; i++ ) {
					...
					//lxm,added,添加自动移动属性
					else if(_tcsicmp(pstrName, _T("automove")) == 0)
					{
						if(_tcsicmp(pstrValue, _T("true")) == 0)
						{
							pManager->SetAutoMove(true);
						}
					}
                }
            }
...
CControlUI* CDialogBuilder::_Parse(CMarkupNode* pRoot, CControlUI* pParent, CPaintManagerUI* pManager)
{
    ...
    for( CMarkupNode node = pRoot->GetChild() ; node.IsValid(); node = node.GetSibling() ) {
		...
		//树控件XML解析
			...
			//lxm,chenged,2015年8月24日，修复了树控件不能动态创建节点的bug
			if( pManager ) {
            ...
            case 8:
                ...
                else if(_tcsicmp(pstrClass,	DUI_CTR_TIMEEDIT) == 0)			 pControl = new  CTimeEditUI;
				break;
            case 9:
				...
				else if( _tcsicmp(pstrClass, DUI_CTR_TRANS_EDIT) == 0)		   pControl = new CTransEditUI;
                break;
            case 10:
            ...
            case 15:
				...
				if( _tcsicmp(pstrClass, DUI_CTR_SLIGHTTABLAYOUT) == 0 )       pControl = new CSlightTabLayout;
				break;
			case 16:
				...
				break;
			case 17:
				if( _tcsicmp(pstrClass, DUI_CTR_SLIPPINGTABLAYOUT) == 0 )       pControl = new CSlippingTabLayoutUI; 
				break;
			case 20:
