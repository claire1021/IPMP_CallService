...
void CRenderEngine::DrawHtmlText(HDC hDC, CPaintManagerUI* pManager, RECT& rc, LPCTSTR pstrText, DWORD dwTextColor, RECT* prcLinks, CDuiString* sLinks, int& nLinkRects, UINT uStyle,int iHeight)
{
	// 考虑到在xml编辑器中使用<>符号不方便，可以使用{}符号代替
	// 支持标签嵌套（如<l><b>text</b></l>），但是交叉嵌套是应该避免的（如<l><b>text</l></b>）
	// The string formatter supports a kind of "mini-html" that consists of various short tags:
	...
	while( *pstrText != _T('\0') ) {
		...
		else
		{
			...
			if( bDraw && bLineDraw ) {
				...
				//wjh,2015/07/16,将文字与图片下对齐，修改为居中
				::TextOut(hDC, ptPos.x,ptPos.y + cyLineHeight - pTm->tmHeight - pTm->tmExternalLeading - iHeight, pstrText, cchSize);
				if( pt.x >= rc.right && (uStyle & DT_END_ELLIPSIS) != 0 ) 
					::TextOut(hDC, ptPos.x + szText.cx, ptPos.y, _T("..."), 3);
			}
			pt.x += szText.cx;
