...
namespace DuiLib
{
	...
	void CContainerUI::DoEvent(TEventUI& event)
	{
		...
		if( m_pVerticalScrollBar != NULL && m_pVerticalScrollBar->IsVisible() && m_pVerticalScrollBar->IsEnabled() 
			//lxm,add,2016-2-2 ,由于显示系统选项卡把滚动条高度设为0，而没有隐藏，还希望它不能拖动，故在此过滤掉
				&&m_pVerticalScrollBar->GetFixedWidth() != 0
				//lxm,end
				)
		{ 
		...
		{ 
			...
			else if(event.Type == UIEVENT_MOUSEMOVE)
			{
				...
				{ 
					SIZE sz = GetScrollPos();
					sz.cx -= (event.ptMouse.x - m_nLastX);
					SetScrollPos(sz);
					m_nLastX = event.ptMouse.x;
				}
			}
	...
	void CContainerUI::SetScrollPos(SIZE szPos)
	{
		int cx = 0;
		int cy = 0;
		//lxm,deleted,2015年7月31日14:31:54,这里判断目的点为0则不滚动是错误的
		//如果有这个判断，则当滚动条正好剩余能够滚动一格时就不会滚动了
		//即滚动条向下滚动，但是没有到达底部，然后晚上滚，则滚不到起始位置。
		if( m_pVerticalScrollBar && m_pVerticalScrollBar->IsVisible() /*&& szPos.cy != 0*/) {
			int iLastScrollPos = m_pVerticalScrollBar->GetScrollPos(); 
			m_pVerticalScrollBar->SetHot();
			m_pVerticalScrollBar->SetScrollPos(szPos.cy);
			cy = m_pVerticalScrollBar->GetScrollPos() - iLastScrollPos;
		}

		if( m_pHorizontalScrollBar && m_pHorizontalScrollBar->IsVisible() /*&& szPos.cx != 0*/) {
			int iLastScrollPos = m_pHorizontalScrollBar->GetScrollPos();
			m_pHorizontalScrollBar->SetHot();
	...
	void CContainerUI::SetPos(RECT rc, bool bNeedInvalidate)
	{
		...
		//lxm,delete,2015年7月31日09:48:30，因为把滚动条改到了容器内，所以这里不能再减掉滚动条的位置
	...
	void CContainerUI::DoPaint(HDC hDC, const RECT& rcPaint)
	{
		...
		if( m_items.GetSize() > 0 ) {
			...
			rc.right -= m_rcInset.right;
			rc.bottom -= m_rcInset.bottom;
			//LXM,delete,2015年7月31日09:47:15，因为滚动条为透明，所以需要把背景绘制到边缘
			// 			if( m_pVerticalScrollBar && m_pVerticalScrollBar->IsVisible() ) rc.right -= m_pVerticalScrollBar->GetFixedWidth();
			// 			if( m_pHorizontalScrollBar && m_pHorizontalScrollBar->IsVisible() ) rc.bottom -= m_pHorizontalScrollBar->GetFixedHeight();
	...
	{
		// 		if( m_pHorizontalScrollBar != NULL && m_pHorizontalScrollBar->IsVisible() ) {
		// 			//lxm,chenged,2015年7月13日09:27:18	//原来滚动条为在容器外，现改为在里边
		// 			//RECT rcScrollBarPos = { rc.left, rc.bottom, rc.right, rc.bottom + m_pHorizontalScrollBar->GetFixedHeight()};
		// 			RECT rcScrollBarPos = { rc.left, rc.bottom - m_pHorizontalScrollBar->GetFixedHeight(), rc.right - ((m_pVerticalScrollBar == NULL)?0:m_pVerticalScrollBar->GetFixedWidth()), rc.bottom };
		// 			
				...
				{ 
					...
					else
					{
						...
						if( m_pHorizontalScrollBar->GetScrollRange() != cxScroll ) {
							...
							//lxm,如果滚动条滚动位置大于Range的话，会将滚动条置于最大位置，但是这里需要通知父窗体 
							if(::abs(cxScroll) <iScrollPos)
							{
								if(m_pManager)
									m_pManager->SendNotify(this, DUI_MSGTYPE_SCROLL,  ::abs(cxScroll) - iScrollPos ,0);
							} 
							//lxm,end
			...
			{ 
				...
				{ 
					...
					else
					{
						//lxm,chenged,2015年7月13日09:27:18	//原来滚动条为在容器外，现改为在里边
						//RECT rcScrollBarPos = { rc.right, rc.top, rc.right + m_pVerticalScrollBar->GetFixedWidth(), rc.bottom };
						RECT rcScrollBarPos = { rc.right - m_pVerticalScrollBar->GetFixedWidth(), rc.top, rc.right, rc.bottom -((m_pHorizontalScrollBar == NULL || m_pHorizontalScrollBar->IsVisible())?0:m_pHorizontalScrollBar->GetFixedHeight()) };
						...
						if( m_pVerticalScrollBar->GetScrollRange() != cyScroll ) {
							...
							//lxm,如果滚动条滚动位置大于Range的话，会将滚动条置于最大位置，但是这里需要通知父窗体 
							if(::abs(cyScroll) <iScrollPos)
							{
								if(m_pManager)
									m_pManager->SendNotify(this, DUI_MSGTYPE_SCROLL,  ::abs(cyScroll) - iScrollPos ,0);
							} 
							//lxm,end
	...
	void CContainerUI::SetScrollBarHot()
	{
		if( m_pVerticalScrollBar != NULL && m_pVerticalScrollBar->IsVisible() && m_pVerticalScrollBar->IsEnabled() )
		{ 
			m_pVerticalScrollBar->SetHot(); 
